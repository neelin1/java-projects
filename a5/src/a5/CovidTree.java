 ...
 * Name(s): Zane Neelin, Aldin Aksay
 * What I thought about this assignment:
 *  Took a lot more time than earlier assignments. Also, far more difficult. Recursion is hard
 *
 */
...
public class CovidTree {
    ...
    public CovidTree insert(Human p, Human c) throws IllegalArgumentException {
        ...
        if (p == null || c == null || node(p) == null || node(c) != null) {
            throw new IllegalArgumentException();
        }
        CovidTree child= new CovidTree(c);
        node(p).children.add(child);
        return child;
    }
    ...
    public List<Human> CovidRouteTo(Human c) {
        ...
        if (human.equals(c)) {
            LinkedList<Human> l1= new LinkedList<>();
            l1.addFirst(c);
            return l1;
        }
        for (CovidTree dt : children) {
            LinkedList<Human> l2= (LinkedList<Human>) dt.CovidRouteTo(c);
            if (l2 != null) {
                l2.addFirst(human);
                return l2;
            }
    ...
    public Human commonAncestor(Human child1, Human child2) {
        ...
        for (int i= obj.length - 1; i >= 0; i-- ) {
            if (l2.contains(obj[i])) { return (Human) obj[i]; }
        }
        return null;
    }
    ...
    public boolean equals(Object ob) {
        ...
        CovidTree ob1= (CovidTree) ob;
        if (human() != ob1.human()) { return false; }
        if (size() != ob1.size()) { return false; }
        for (CovidTree dt : children) {
            if (!isInSet(dt, ob1.children)) return false;
        }
        return true;
